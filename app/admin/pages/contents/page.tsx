...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/pages/contents safeQuery]", err);
    return [];
  }
}
...
export default async function PageContentsPage({
  ...
  const data = await safeQuery<PageRow>(
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Page Title</th>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-blue-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deletePage.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
