...
export default function DashboardClient({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (
        (openMenu === "student" && studentMonthRef.current?.contains(t)) ||
        (openMenu === "college" && collegeMonthRef.current?.contains(t))
      ) return;
      setOpenMenu(null);
    };
  ...
  const monthIndex = useMemo(() => {
    ...
    return MONTH_OPTIONS.indexOf(monthFilter);
  }, [monthFilter]);
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        {statCards.map((stat, i) => (
          <div key={i} className="bg-white rounded-[5px] p-6 border border-slate-100 shadow-md relative overflow-hidden transition-all hover:shadow-lg">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-1">{stat.title}</p>
                <p className="text-3xl font-bold text-slate-900">{stat.value}</p>
              </div>
              <div className="bg-[#FF3C3C] p-3 rounded-lg text-white shadow-sm">
                {stat.icon}
              </div>
            </div>
            <div className="mt-4">
      ...
      {/* Transaction Overview Pie Charts */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-[5px] border border-slate-100 shadow-md p-6">
          ...
          <div className="h-[300px] w-full">
            <TransactionsPieChart data={studentTransactionPie} />
          </div>
        </div>
        <div className="bg-white rounded-[5px] border border-slate-100 shadow-md p-6">
          <h2 className="text-[20px] font-bold text-slate-800 mb-6">College Transactions</h2>
          <div className="h-[300px] w-full">
            <TransactionsPieChart data={collegeTransactionPie} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-[5px] border border-slate-100 shadow-md p-6 xl:col-span-1">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-[22px] font-semibold text-slate-800">Recent Activity</h2>
          </div>
          <div className="space-y-6">
            ...
            {recentActivity.length === 0 && (
              <p className="text-center text-slate-400 py-10 italic">No recent activities.</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[5px] border border-slate-100 shadow-md overflow-hidden flex flex-col p-6 xl:col-span-2">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/admin/students/profile" className="text-[13px] font-bold text-blue-600 hover:text-blue-700 underline">
              View All Students
            </Link>
          </div>
