...
async function deleteTag(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM ask_question_tags WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/academic/ask-tags safeQuery]", err);
    return [];
  }
}
...
export default async function AskTagsPage({
  searchParams,
}: {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-violet-600 text-[22px]" style={ICO_FILL}>tag</span>
            ASK Question Tags
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage tags for user questions.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Tag Name</th>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-violet-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteTag.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
