...
export default async function CareerCoursesPage({
  ...
  const where = q ? "WHERE courseName LIKE ? OR jobProfiles LIKE ?" : "";
  const params = q ? [`%${q}%`, `%${q}%`] : [];

  const data = await safeQuery<CourseRow>(
    `SELECT id, courseName, jobProfiles, avgSalery
     FROM counseling_courses_job_careers
     ${where}
     ORDER BY id DESC
     LIMIT 100`,
    params
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-cyan-600 text-[22px]" style={ICO_FILL}>school</span>
            Career Courses Details
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage courses and related job prospects.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Course Name</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Avg. Salary</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Job Profiles</th>
              ...
              {data.length === 0 ? (
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-cyan-50/20 transition-colors group">
                    <td className="px-5 py-4 font-bold text-slate-800">{r.courseName}</td>
                    ...
                    <td className="px-4 py-4">
                      <p className="text-[10px] text-slate-400 line-clamp-2 max-w-[400px]">
                        {r.jobProfiles || "No profiles listed"}
                      </p>
                    </td>
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteCourse.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
