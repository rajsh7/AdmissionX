...
export default async function StudentPaymentsPage({
  ...
  function buildUrl(overrides: Record<string, string | number>) {
    ...
    return `/admin/payment/student${qs ? `?${qs}` : ""}`;
  }
    ...
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      <div>
        <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
          <span className="material-symbols-rounded text-emerald-600 text-[22px]" style={ICO_FILL}>person</span>
          Student Payments
        </h1>
        <p className="text-sm text-slate-500 mt-0.5">Students paying colleges for application fees</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex flex-col sm:flex-row gap-3 items-center">
        <form method="GET" action="/admin/payment/student" className="flex-1 w-full">
