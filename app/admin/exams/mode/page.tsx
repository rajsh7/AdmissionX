...
async function deleteMode(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM examination_modes WHERE id = ?", [id]);
  } catch (e) {
    console.error("[admin/exams/mode deleteAction]", e);
  }
  revalidatePath("/admin/exams/mode");
...
async function safeQuery<T >(
  ...
  try {
    ...
    console.error("[admin/exams/mode safeQuery]", err);
    return [];
  }
}
...
export default async function ExamModePage({
  ...
  const docs = await db.collection("examination_modes").find(filter).sort({ id: -1 }).limit(100).toArray();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-indigo-600 text-[22px]" style={ICO_FILL}>settings_accessibility</span>
            Examination Mode
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage the format of exams (e.g., CBT, Pen & Paper).</p>
        </div>
        <form method="GET" className="relative max-w-sm w-full">
           ...
           <input 
             name="q" 
             defaultValue={q}
             placeholder="Search modes or slugs..." 
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Mode Name</th>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-indigo-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteMode.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
