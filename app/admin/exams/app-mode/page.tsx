...
async function deleteMode(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM application_modes WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/exams/app-mode safeQuery]", err);
    return [];
  }
}
...
export default async function AppModePage({
  searchParams,
}: {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-blue-600 text-[22px]" style={ICO_FILL}>app_registration</span>
            Application Mode
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage modes of exam applications (e.g., Online, Offline).</p>
        </div>
