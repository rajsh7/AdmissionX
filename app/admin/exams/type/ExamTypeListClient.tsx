...
interface ExamTypeListClientProps {
  data: ExamTypeRow[];
...
export default function ExamTypeListClient({
  data,
  createAction,
  updateAction,
  deleteAction,
}: ExamTypeListClientProps) {
  const [isAddOpen, setIsAddOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<ExamTypeRow | null>(null);
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div className="relative flex-1 max-w-sm w-full text-slate-400">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500 transition-all font-medium text-slate-800"
          />
        </div>
        <button
          onClick={() => setIsAddOpen(true)}
          className="flex items-center gap-2 px-5 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-cyan-500/20 transition-all active:scale-95 flex-shrink-0"
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-left">
                <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Examination Type</th>
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-2 opacity-30">
                      ...
                      <p className="font-bold">No Exam Types Found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                data.map((item) => (
                  <tr key={item.id} className="hover:bg-cyan-50/20 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-cyan-100 flex items-center justify-center text-cyan-600 group-hover:bg-cyan-600 group-hover:text-white transition-all duration-300">
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                       <button
                         ...
                         className="p-2 text-slate-400 hover:text-cyan-600 hover:bg-white rounded-lg transition-all border border-transparent hover:border-cyan-100 hover:shadow-sm"
                         title="Edit Type"
                       >
