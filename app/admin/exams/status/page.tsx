...
async function deleteStatus(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM application_and_exam_statuses WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T >(
  ...
  try {
    ...
    console.error("[admin/exams/status safeQuery]", err);
    return [];
  }
}
...
export default async function ExamStatusPage({
  searchParams,
}: {
  ...
  const docs = await db.collection("application_and_exam_statuses").find(filter).sort({ id: -1 }).limit(100).toArray();
  const data: StatusRow[] = docs.map((d: any) => ({
    id: Number(d.id ?? 0),
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-sky-600 text-[22px]" style={ICO_FILL}>rule</span>
            Application & Exam Status
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage statuses for exam enrollment and applications.</p>
        </div>
        <form method="GET" className="relative max-w-sm w-full">
           ...
           <input 
             name="q" 
             defaultValue={q}
             placeholder="Search statuses or slugs..." 
             className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20"
           />
        </form>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                ...
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center">State</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {data.length === 0 ? (
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-sky-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteStatus.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
