...
async function deleteCriteria(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM eligibility_criterias WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T >(
  ...
  try {
    ...
    console.error("[admin/exams/eligibility safeQuery]", err);
    return [];
  }
}
...
export default async function EligibilityCriteriaPage({
  searchParams,
}: {
  ...
  const docs = await db.collection("eligibility_criterias").find(filter).sort({ id: -1 }).limit(100).toArray();
  const data: CriteriaRow[] = docs.map((d: any) => ({
    id: Number(d.id ?? 0),
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-violet-600 text-[22px]" style={ICO_FILL}>assignment_turned_in</span>
            Eligibility Criteria
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage eligibility requirements and criteria patterns.</p>
        </div>
        <form method="GET" className="relative max-w-sm w-full">
           ...
           <input 
             name="q" 
             defaultValue={q}
             placeholder="Search criteria or slugs..." 
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Criteria Name</th>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-violet-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteCriteria.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
