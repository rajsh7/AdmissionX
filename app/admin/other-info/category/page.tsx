...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/other-info/category safeQuery]", err);
    return [];
  }
}
...
export default async function CategoryPage({
  ...
  const data = await safeQuery<CategoryRow>(
    `SELECT id, name
     FROM category
     ${where}
     ORDER BY id DESC
    ...
    <div className="p-6 space-y-6 max-w-[1400px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage general system categories and classifications.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Category Name</th>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-amber-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteCategory.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
