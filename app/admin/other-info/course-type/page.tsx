...
async function deleteCourseType(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM coursetype WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/other-info/course-type safeQuery]", err);
    return [];
  }
}

interface CourseTypeRow extends RowDataPacket {
  id: number;
  name: string;
}
...
export default async function CourseTypePage({
  ...
  const data = await safeQuery<CourseTypeRow>(
    `SELECT id, name
     FROM coursetype
     ${where}
     ORDER BY name ASC
    ...
    <div className="p-6 space-y-6 max-w-[1400px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage different course formats (e.g., Full-time, Part-time, Online).</p>
        </div>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-blue-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteCourseType.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
