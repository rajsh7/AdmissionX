...
async function deleteCourse(id: number) {
  ...
  try {
    ...
    console.error("[admin/other-info/courses deleteAction]", e);
  }
  revalidatePath("/admin/other-info/courses");
}
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/other-info/courses safeQuery]", err);
    return [];
  }
}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-emerald-600 text-[22px]" style={ICO_FILL}>auto_stories</span>
            Courses
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage the master list of courses available in the system.</p>
        </div>
