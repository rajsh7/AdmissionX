...
export default function OtherCourseListClient({
  data,
  total,
  page,
  totalPages,
              ...
              {data.length === 0 ? (
                <tr>
                  <td colSpan={3} className="py-24 text-center">
                    <span className="material-symbols-rounded text-6xl text-slate-200 mb-4 block" style={ICO_FILL}>auto_stories</span>
                  ...
                  <tr key={r.id} className="hover:bg-emerald-50/20 transition-colors group">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-emerald-50 flex items-center justify-center flex-shrink-0 text-emerald-600">
                          <span className="material-symbols-rounded text-[16px]" style={ICO_FILL}>menu_book</span>
                        </div>
      ...
      {showMore && (
        <div className="py-8 flex flex-col items-center gap-1">
          <button
            ...
            className="group flex flex-col items-center gap-1 text-slate-400 hover:text-emerald-600 transition-colors"
          >
            <span className="text-xs font-black uppercase tracking-[0.2em] ml-[0.2em]">
              Show More ({data.length - visibleCount} remaining)
            </span>
            <span className="material-symbols-outlined text-[40px] animate-bounce group-hover:text-emerald-600">
              keyboard_arrow_down
            </span>
          </button>
        </div>
