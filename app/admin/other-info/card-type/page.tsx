...
async function deleteCardType(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM cardtype WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/other-info/card-type safeQuery]", err);
    return [];
  }
}

interface CardTypeRow extends RowDataPacket {
  id: number;
  name: string;
}
...
export default async function CardTypePage({
  searchParams,
}: {
  ...
  const data = await safeQuery<CardTypeRow>(
    `SELECT id, name
     FROM cardtype
     ${where}
     ORDER BY id DESC
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-blue-600 text-[22px]" style={ICO_FILL}>credit_card</span>
            Card Type
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage payment card types (e.g., Visa, Mastercard, Debit).</p>
        </div>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-blue-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteCardType.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
