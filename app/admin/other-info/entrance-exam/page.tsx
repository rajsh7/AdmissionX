...
async function deleteEntranceExam(id: number) {
  "use server";
  try {
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/other-info/entrance-exam safeQuery]", err);
    return [];
  }
}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-rose-600 text-[22px]" style={ICO_FILL}>quiz</span>
            Entrance Exams
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage the list of various entrance examinations.</p>
        </div>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-rose-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteEntranceExam.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
