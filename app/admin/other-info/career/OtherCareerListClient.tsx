...
export default function OtherCareerListClient({
  data,
  total,
  page,
  totalPages,
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Applicant</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Applied For</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Contact Info</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Applied On</th>
              ...
              {data.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-24 text-center">
                    ...
                    <p className="text-slate-500 font-semibold text-sm">No career applications found.</p>
                  </td>
                </tr>
              ) : (
                data.slice(0, visibleCount).map((r, idx) => (
                  <tr key={r.id} className="hover:bg-pink-50/20 transition-colors group">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex flex-col">
                        ...
                        {r.gender && (
                          <span className="text-[10px] text-slate-400 mt-0.5">{r.gender}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                         <div className="px-2 py-0.5 rounded bg-slate-50 border border-slate-100 text-[10px] font-bold text-slate-600 uppercase tracking-wider">
                           {r.post || "General"}
                         </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 text-xs text-slate-400 font-medium italic">
      ...
      {showMore && (
        <div className="py-8 flex flex-col items-center gap-1">
          <button
            ...
            className="group flex flex-col items-center gap-1 text-slate-400 hover:text-pink-600 transition-colors"
          >
            <span className="text-xs font-black uppercase tracking-[0.2em] ml-[0.2em]">
              Show More ({data.length - visibleCount} remaining)
            </span>
            <span className="material-symbols-outlined text-[40px] animate-bounce group-hover:text-pink-600">
              keyboard_arrow_down
            </span>
          </button>
        </div>
      ...
      {showPagination && (
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/30 rounded-b-2xl">
          <p className="text-xs text-slate-500 font-medium tracking-tight">
            Showing <span className="text-slate-800 font-bold">{(page - 1) * pageSize + 1}–{Math.min((page - 1) * pageSize + pageSize, total)}</span> of <span className="text-slate-800 font-bold">{total.toLocaleString()}</span> applications
          </p>
