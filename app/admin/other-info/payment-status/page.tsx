...
async function deletePaymentStatus(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM paymentstatus WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/other-info/payment-status safeQuery]", err);
    return [];
  }
}
...
export default async function PaymentStatusPage({
  ...
  const data = await safeQuery<PaymentStatusRow>(
    `SELECT id, name
     FROM paymentstatus
     ${where}
     ORDER BY id DESC
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-emerald-600 text-[22px]" style={ICO_FILL}>monetization_on</span>
            Payment Status
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage payment status definitions (e.g., Paid, Failed, Processing).</p>
        </div>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-emerald-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deletePaymentStatus.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
