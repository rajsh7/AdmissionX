...
async function deleteCollegeType(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM collegetype WHERE id = ?", [id]);
  } catch (e) {
...
async function safeQuery<T extends RowDataPacket>(
  ...
  try {
    ...
    console.error("[admin/other-info/college-type safeQuery]", err);
    return [];
  }
}

interface CollegeTypeRow extends RowDataPacket {
  id: number;
  name: string;
}
...
export default async function CollegeTypePage({
  searchParams,
}: {
  ...
  const data = await safeQuery<CollegeTypeRow>(
    `SELECT id, name
     FROM collegetype
     ${where}
     ORDER BY id DESC
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-indigo-600 text-[22px]" style={ICO_FILL}>domain</span>
            College Type
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage classifications of colleges (e.g., Private, Government, Deemed).</p>
        </div>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-indigo-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteCollegeType.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
