...
async function safeQuery<T >(
  ...
  try {
    ...
    console.error("[admin/other-info/facilities safeQuery]", err);
    return [];
  }
}
...
export default async function FacilitiesPage({
  ...
  const data = await safeQuery<FacilityRow>(
    `SELECT id, name
     FROM facilities
     ${where}
     ORDER BY name ASC
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="material-symbols-rounded text-amber-600 text-[22px]" style={ICO_FILL}>apartment</span>
            Campus Facilities
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage the master list of facilities available at colleges.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-left">
                <th className="px-5 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Facility Name</th>
                ...
                data.map((r) => (
                  <tr key={r.id} className="hover:bg-amber-50/20 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-right">
                       <DeleteButton action={deleteFacility.bind(null, r.id)} size="sm" />
                    </td>
                  </tr>
                ))
              )}
