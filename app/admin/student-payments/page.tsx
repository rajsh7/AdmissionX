...
import StudentPaymentListClient from "./StudentPaymentListClient";
...
export default async function StudentPaymentsPage({
  ...
  const [applications, total, stats] = await Promise.all([
    ...
    db.collection("applications").aggregate([
      { $match: filter },
      {
        $group: {
          _id: null,
          total_amount: { $sum: { $ifNull: ["$amount_paid", 0] } },
  ...
  function buildUrl(overrides: Record<string, string | number>) {
    ...
    return `/admin/student-payments${qs ? `?${qs}` : ""}`;
  }
    ...
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      <div>
        <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
          <span className="material-symbols-rounded text-emerald-600 text-[22px]" style={ICO_FILL}>payments</span>
          Student Payments
        </h1>
        <p className="text-sm text-slate-500 mt-0.5">All student payments for college applications</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Payments", value: total.toLocaleString("en-IN"), icon: "receipt_long", color: "bg-indigo-50 text-indigo-600" },
          { label: "Total Amount", value: `₹${statsData.total_amount.toLocaleString("en-IN")}`, icon: "payments", color: "bg-emerald-50 text-emerald-600" },
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex flex-col sm:flex-row gap-3 items-center">
        <form method="GET" action="/admin/student-payments" className="flex-1 w-full">
          <div className="relative">
            ...
            <input name="q" defaultValue={q} placeholder="Search by application ref, transaction ID, college, course..."
