...
async function createScholarship(formData: FormData) {
  "use server";
  try { await pool.query(`INSERT INTO college_scholarships (collegeprofile_id, title, description, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())`, [formData.get("collegeprofile_id"), formData.get("title"), formData.get("description") || null]); }
...
async function updateScholarship(formData: FormData) {
  "use server";
  try { await pool.query(`UPDATE college_scholarships SET collegeprofile_id=?, title=?, description=?, updated_at=NOW() WHERE id=?`, [formData.get("collegeprofile_id"), formData.get("title"), formData.get("description") || null, formData.get("id")]); }
...
async function deleteScholarshipRow(id: number) {
  "use server";
  try { await pool.query("DELETE FROM college_scholarships WHERE id = ?", [id]); }
...
async function safeQuery<T>(sql: string, params: (string | number)[] = []): Promise<T[]> {
  ...
  catch (err) { console.error("[admin/colleges/scholarships safeQuery]", err); return []; }
}

interface ScholarshipRow { id: number; collegeprofile_id: number; college_name: string; title: string; description: string; }
...
export default async function CollegeScholarshipsPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  if (q) { conditions.push("(u.firstname LIKE ? OR s.title LIKE ? OR s.description LIKE ?)"); params.push(`%${q}%`, `%${q}%`, `%${q}%`); }
  if (collegeId) { conditions.push("s.collegeprofile_id = ?"); params.push(collegeId); }
  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";

  const [scholarships, countRows, collegeOptions] = await Promise.all([
    safeQuery<ScholarshipRow>(
      `SELECT s.id, s.collegeprofile_id, COALESCE(u.firstname,'Unnamed College') as college_name, s.title, s.description
       FROM college_scholarships s JOIN collegeprofile cp ON cp.id=s.collegeprofile_id JOIN users u ON u.id=cp.users_id ${where} ORDER BY s.created_at DESC LIMIT ? OFFSET ?`,
      [...params, PAGE_SIZE, offset],
    ),
    safeQuery<CountRow>(`SELECT COUNT(*) AS total FROM college_scholarships s JOIN collegeprofile cp ON cp.id=s.collegeprofile_id JOIN users u ON u.id=cp.users_id ${where}`, params),
    fetchCollegeOptions(),
  ]);
  ...
  function pageUrl(p: number) {
    ...
    return `/admin/colleges/scholarships?${qs.toString()}`;
  }

  return (
    <div className="p-6 space-y-6 max-w-[1400px]">
      <CollegeFilterBar colleges={collegeOptions} selectedId={collegeId} total={total} label="College Scholarships" icon="workspace_premium" description="Manage financial aid and scholarship programs — filter by college to see classified data." />
      <ScholarshipListClient scholarships={scholarships} colleges={collegeOptions as any} offset={offset} onAdd={createScholarship} onEdit={updateScholarship} onDelete={deleteScholarshipRow} />
      {totalPages > 1 && (
