...
async function safeQuery<T>(sql: string, params: (string | number)[] = []): Promise<T[]> {
  try {
    ...
    console.error("[admin/colleges/scholarships/[id] safeQuery]", err);
    return [];
  }
}
...
async function updateScholarship(formData: FormData) {
  ...
  try {
    ...
    console.error("[admin/colleges/scholarships/[id] updateAction]", e);
  }
  revalidatePath("/admin/colleges/scholarships");
  redirect("/admin/colleges/scholarships");
}
...
export default async function EditScholarshipPage({
  ...
  const [scholarships, colleges] = await Promise.all([
  ...
  const createdAt = scholarship.created_at
    ? new Date(scholarship.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })
    : "-";

  return (
    <div className="min-h-screen bg-slate-50/60 p-4 sm:p-6 lg:p-8">
      <form action={updateScholarship} className="w-full">
        <input type="hidden" name="id" value={scholarship.id} />

        <div className="flex items-center gap-3 mb-8">
          <Link
            href="/admin/colleges/scholarships"
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-6 items-start">
          <div className="flex-1 min-w-0 space-y-6">
            <Card>
              <SectionHeading icon="redeem" title="Scholarship Details" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className={labelCls}>College</label>
                  <div className="relative">
                    <select name="collegeprofile_id" defaultValue={scholarship.collegeprofile_id} className={selectCls}>
                ...
                <div>
                  <label className={labelCls}>Scholarship Title</label>
                  <input
                    name="title"
            ...
            <Card>
              ...
              <textarea
                ...
                placeholder="Eligibility, benefits, and requirements..."
          ...
          <div className="w-full space-y-6">
            <Card>
              <SectionHeading icon="tune" title="Program Summary" />
              <div className="space-y-3">
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">College</p>
                  <p className="text-sm font-semibold text-slate-700 mt-1">
                    {scholarship.college_name || "-"}
                  </p>
                </div>
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Scholarship</p>
                  <p className="text-sm font-semibold text-slate-700 mt-1">
                    {scholarship.title || "-"}
                  </p>
                </div>
