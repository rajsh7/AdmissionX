...
async function deleteFacultyRow(id: number) {
  "use server";
  try {
    await pool.query("DELETE FROM faculty WHERE id = ?", [id]);
  } catch (e) {
    console.error("[admin/colleges/faculty deleteAction]", e);
  }
...
const DEFAULT_AVATAR = "https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&q=80&w=150";
...
export default async function CollegeFacultyPage({
  ...
  if (q) {
    conditions.push(
      "(f.name LIKE ? OR u.firstname LIKE ?)",
    );
    params.push(`%${q}%`, `%${q}%`);
  }
  ...
  // ── Query faculty ─────────────────────────────────────────────────────────
  const [facultyMembers, countRows] = await Promise.all([
    ...
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <form method="GET" action="/admin/colleges/faculty" className="w-full sm:w-80">
            <div className="relative">
              ...
              <input 
                ...
                name="q" 
                defaultValue={q}
                placeholder="Search faculty, colleges..." 
                  ...
                  <tr key={f.id} className="hover:bg-blue-50/20 transition-colors">
                    ...
                    <td className="px-4 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <DeleteButton action={deleteFacultyRow.bind(null, f.id)} size="sm" />
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100 bg-slate-50/50">
            <p className="text-xs text-slate-500">
              Showing <strong>{offset + 1}–{Math.min(offset + PAGE_SIZE, total)}</strong> of <strong>{total.toLocaleString()}</strong> faculty members
            </p>
            <div className="flex items-center gap-1">
              {page > 1 ? (
                <Link href={`/admin/colleges/faculty?page=${page - 1}${q ? `&q=${q}` : ''}`} className="px-3 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">← Prev</Link>
              ) : (
              ...
              {page < totalPages ? (
                <Link href={`/admin/colleges/faculty?page=${page + 1}${q ? `&q=${q}` : ''}`} className="px-3 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">Next →</Link>
              ) : (
