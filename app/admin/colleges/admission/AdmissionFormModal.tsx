...
export default function AdmissionFormModal({
  ...
  return (
    <AdminModal
      isOpen={isOpen}
      onClose={onClose}
      title={admission ? "Edit Admission Procedure" : "Add Admission Procedure"}
    >
      <form action={handleAction} className="space-y-4 px-1">
        {admission && <input type="hidden" name="id" value={admission.id} />}
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              defaultValue={admission?.collegeprofile_id || ""}
              required
              className={SELECT}
            >
        ...
        {/* Title */}
        <div>
          <label className={LABEL}>Procedure Title *</label>
          <input
            name="title"
            defaultValue={admission?.title || ""}
            placeholder="e.g. B.Tech Admission Process 2024"
            required
            className={INPUT}
          />
        </div>
        ...
        <div className="flex gap-3 pt-4 sticky bottom-0 bg-white pb-1">
          ...
          >
            {isPending ? "Saving…" : admission ? "Update Procedure" : "Add Procedure"}
          </button>
        </div>
      </form>
    </AdminModal>
