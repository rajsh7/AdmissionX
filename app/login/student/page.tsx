...
function StudentLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        const destination = data.user?.id ? `/dashboard/student/${data.user.id}` : redirectTo;
          ...
          <div className="w-[38%] relative flex-shrink-0" style={{ minHeight: "520px" }}>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 z-20">
              <h2 className="text-white text-xl font-bold leading-snug mb-3">
                Welcome back to<br />
                <span className="font-light">AdmissionX</span>
              </h2>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/15 rounded-xl px-3 py-2">
                  ...
                  <p className="text-white text-[10px] font-medium leading-tight">
                    500+ colleges<br /><span className="text-white/60 font-normal">across India</span>
                  </p>
                </div>
                <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/15 rounded-xl px-3 py-2">
                  <span className="material-symbols-outlined text-white text-[14px]" style={{ fontVariationSettings: "'FILL' 1" }}>description</span>
                  <p className="text-white text-[10px] font-medium leading-tight">
                    Track applications<br /><span className="text-white/60 font-normal">and get admitted</span>
                  </p>
                </div>
              </div>
            </div>
